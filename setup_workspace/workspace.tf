...
resource "null_resource" "setup_backend_file" {
  ...
  provisioner "local-exec" {
    command = "mv backend.hcl  ..${var.working_directory}"
  }
}
...
# !!!!CAUTION!!!!: Make sure your credential are secured outside version control (and follow secrets mangement bestpractices)
#   
